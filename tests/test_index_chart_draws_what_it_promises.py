...
class TheChartKeepsItsPivots(unittest.TestCase):
    ...
    def test_the_header_and_the_payload_agree(self):
        ...
        self.assertIn("CPR, R1-R4, S1-S4, 20-EMA, Supertrend 10,2", fn)
        self.assertIn('analytics["lines"]', _route_body())
        self.assertIn("_supertrend_overlay_lines(candles)", _route_body())
...
class TheChartDrawsTheSupertrend(unittest.TestCase):
    ...
    def test_the_legend_names_it_once(self):
        ns = _helpers()
        overlays = ns["_supertrend_overlay_lines"](_fake_candles())
        labelled = [o for o in overlays if o["label"]]
        self.assertEqual(len(labelled), 1, "one legend entry, not one per flip")
        self.assertEqual(labelled[0]["label"], "ST 10,2")
